...
const confirmNode = (text: string, auth: keyof IAuthStatus) => (
	<Popconfirm title={`确认${text}吗?`} okText="是" cancelText="否">
		<AuthBtn type="link" r-auth={auth} size="small">
			{text}
		</AuthBtn>
	</Popconfirm>
);
...
export const columns: ProColumns<IArticleItem>[] = [
	...
	},
	{
		...
		render: (_, record) => {
			return (
				<div className={styles.abstract}>{record.abstract || '-'}</div>
			);
		},
	},
	{
			...
			if (record.status === EnumArticleStatus.Draft) {
				return (
					<>
						...
						{confirmNode('上架', 'edit')}
						<Divider type="vertical" />
						{confirmNode('删除', 'delete')}
					</>
				);
			} else if (record.status === EnumArticleStatus.Trash) {
				return (
					<>
						<AuthBtn type="link" r-auth="edit" size="small">
							还原
						</AuthBtn>
						<Divider type="vertical" />
						{confirmNode('删除', 'delete')}
